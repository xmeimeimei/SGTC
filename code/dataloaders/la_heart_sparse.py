...
class LAHeart1(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''if os.path.exists(self._base_dir + "/{}/base_z.h5".format(image_name)):
            h5f = h5py.File(self._base_dir + "/{}/base_z.h5".format(image_name), 'r')
            print("laheart1:",h5f)
            label = h5f['label'][:]
            h5ftrue=h5py.File("/home/ouc/data1/yk/DeSCO-master/data/2018LA_Seg_Training Set" + "/{}/mri_norm2.h5".format(image_name), 'r')
            truelabel=h5ftrue['label'][:]
            label[:, label.shape[1]//2, :] = truelabel[:, label.shape[1]//2, :]
            label[label.shape[0]//2, :, :] = truelabel[label.shape[0]//2, :, :]
        else:'''
        h5f = h5py.File(self._base_dir + "/{}/mri_norm2.h5".format(image_name), 'r')
        label = h5f['label'][:]
        image = h5f['image'][:]
...
class LAHeart2(Dataset):
    ...
    def __getitem__(self, idx):
        image_name = self.image_list[idx]
        '''if os.path.exists(self._base_dir + "/{}/base_w.h5".format(image_name)):
            h5f = h5py.File(self._base_dir + "/{}/base_w.h5".format(image_name), 'r')
            print("laheart2:",h5f)
            label = h5f['label'][:]
            h5ftrue = h5py.File("/home/ouc/data1/yk/DeSCO-master/data/2018LA_Seg_Training Set" + "/{}/mri_norm2.h5".format(image_name), 'r')
            truelabel = h5ftrue['label'][:]
            label[:, :, 32] = truelabel[:, :, 32]
            label[label.shape[0]//2, :, :] = truelabel[label.shape[0]//2, :, :]
        else:'''
        h5f = h5py.File(self._base_dir + "/{}/mri_norm2.h5".format(image_name), 'r')
        label = h5f['label'][:]
...
class LAHeart3(Dataset):
    ...
    def __getitem__(self, idx):
        image_name = self.image_list[idx]
        '''if os.path.exists(self._base_dir_+"/{}/base_h.h5".format(image_name)):
            h5f = h5py.File(self._base_dir_+"/{}/base_h.h5".format(image_name))
            print("laheart3:",h5f)
            label = h5f['label'][:]
            h5ftrue = h5py.File("/home/ouc/data1/yk/DeSCO-master/data/2018LA_Seg_Training Set"+"/{}/mri_norm2.h5".format(image_name),'r')
            truelabel = h5ftrue['label'][:]

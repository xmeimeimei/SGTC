...
def wce(logits,target,weights,batch_size,H,W,D):
    ...
    temp = logp.shape
    # temp2 = target.view(batch_size,1,H,W,D)
    # for i in range(temp2.size(0)):
    #     for j in range(temp2.size(1)):
    #         for k in range(temp2.size(2)):
    #             if temp2[i, j, k] == 2:
    #                 print(f"Found value 2 at index ({i}, {j}, {k})")
